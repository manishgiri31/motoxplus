    ...
    <div>
      <div className="mb-8 flex items-center justify-between">
        ...
        <Link
          ...
          className="flex items-center gap-2 bg-red-600 hover:bg-red-700 text-white font-bold px-4 py-2.5 rounded-sm transition-colors text-sm uppercase tracking-wider"
        >
          <Plus size={16} />
          Add Product
        </Link>
      ...
      <div className="flex flex-wrap gap-2 mb-6">
        <Link
          href="/admin/products"
          className={`px-3 py-2 rounded-sm text-xs font-semibold uppercase tracking-wider transition-colors ${!searchParams.category ? "bg-red-600 text-white" : "glass border border-[var(--border-color)] text-[var(--text-muted)] hover:border-red-600/50"}`}
        >
          All
        </Link>
        {categories.map((cat) => (
          <Link
            ...
            className={`px-3 py-2 rounded-sm text-xs font-semibold uppercase tracking-wider transition-colors ${searchParams.category === cat.id ? "bg-red-600 text-white" : "glass border border-[var(--border-color)] text-[var(--text-muted)] hover:border-red-600/50"}`}
          >
            {cat.name}
          </Link>
        ))}
              ...
              <tr key={product.id} className="hover:bg-white/2 transition-colors">
                <td className="px-4 py-4">
                  <div className="flex items-center gap-3">
                    <div className="relative w-10 h-10 bg-zinc-900 rounded-sm flex-shrink-0 overflow-hidden">
                      {(() => {
                ...
                <td className="px-4 py-4">
                  <span className={`text-xs font-semibold uppercase tracking-wider px-2 py-1 rounded-sm ${product.isActive ? "bg-green-900/20 text-green-400" : "bg-red-900/20 text-red-400"}`}>
