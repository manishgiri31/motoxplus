...
export async function PATCH(req: NextRequest, { params }: { params: { id: string; vinId: string } }) {
...
export async function DELETE(_req: NextRequest, { params }: { params: { id: string; vinId: string } }) {
  if (!(await requireAdmin())) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
