...
// Same flow as the dealer /dealer/orders/[id]/pay-upi page — /api/payments/upi/*
// is channel-agnostic (see resolveOrderActor), so this is a near-duplicate
// with only the portal paths and the account-holder prefill source changed.
export default function AccountPayUpiPage() {
  ...
  useEffect(() => {
    fetch(`/api/payments/upi/${id}`)
      ...
      .then((data) => {
        ...
        const email = data.order?.customer?.user?.email;
        if (email && !email.endsWith("@phone.motoxplus.invalid")) {
          setForm((f) => ({ ...f, email }));
        }
        if (data.order?.deliveryName) {
          setForm((f) => ({ ...f, name: data.order.deliveryName }));
        }
        if (data.order?.deliveryPhone) {
          setForm((f) => ({ ...f, phone: data.order.deliveryPhone }));
        }
      })
  ...
  if (!order || !settings) {
    return (
      <div className="text-center py-20">
        ...
        <Link href="/account/orders" className="text-red-400 hover:text-red-300 text-sm mt-2 inline-block">← Back to orders</Link>
      </div>
    );
  }
  ...
  if (alreadyPaid) {
    return (
      <div className="max-w-lg mx-auto text-center py-16">
        ...
        <Link href={`/account/orders/${id}`} className="inline-flex items-center gap-2 bg-red-600 hover:bg-red-700 text-white font-bold py-3 px-6 rounded-xl text-sm transition-colors">
          View Order
        </Link>
      </div>
    );
  ...
  if (submitted || (existingSubmission && ["SUBMITTED", "UNDER_REVIEW"].includes(existingSubmission.status))) {
    return (
      <div className="max-w-lg mx-auto text-center py-16">
        ...
        <Link href={`/account/orders/${id}`} className="inline-flex items-center gap-2 border border-[var(--border-color)] text-[var(--text-muted)] hover:text-[var(--text-primary)] font-bold py-3 px-6 rounded-xl text-sm transition-colors">
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="mb-6">
        <Link href={`/account/orders/${id}`} className="inline-flex items-center gap-2 text-[var(--text-muted)] hover:text-[var(--text-primary)] text-sm mb-4 transition-colors">
          <ArrowLeft size={14} /> Back to Order
        </Link>
      ...
      <div className="glass border border-[var(--border-color)] rounded-2xl p-5 mb-5">
        ...
        <div className="space-y-2 text-sm mb-4">
          ...
          <div className="border-t border-[var(--border-color)] pt-3 flex justify-between font-bold">
            <span className="text-[var(--text-primary)]">Grand Total (incl. taxes)</span>
