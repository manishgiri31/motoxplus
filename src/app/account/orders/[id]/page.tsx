...
export default async function AccountOrderDetailPage(
  ...
  const awaitingPayment = order.paymentStatus === "PENDING" && order.amountDue > 0;
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <div className="mb-8 flex items-start justify-between">
        <div>
          <h1 className="text-3xl font-black text-[var(--text-primary)] tracking-tight font-mono">{order.orderNumber}</h1>
      ...
      {awaitingPayment && (
        <Link
          href={`/account/orders/${order.id}/pay-upi`}
          className="block mb-6 bg-red-600 hover:bg-red-700 text-white font-bold text-center py-3 rounded-sm transition-colors text-sm uppercase tracking-wider"
        >
          Complete Payment — {formatCurrency(order.amountDue)}
        </Link>
      )}
          ...
          <div className="glass border border-white/5 rounded-sm p-6">
            ...
            <div className="space-y-3 text-sm">
              <div className="flex justify-between">
                ...
                <span className="text-[var(--text-primary)]">{order.shippingCost > 0 ? formatCurrency(order.shippingCost) : "Free"}</span>
              </div>
