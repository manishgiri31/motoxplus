...
export default async function AccountOrdersPage(props: { searchParams: Promise<{ page?: string }> }) {
  ...
  return (
    <div>
      <div className="mb-8">
        ...
        <p className="text-[var(--text-muted)] mt-1">{total} total order{total !== 1 ? "s" : ""}</p>
      </div>
      ...
      {orders.length === 0 ? (
        <div className="text-center py-20 glass border border-[var(--border-color)] rounded-xl">
          ...
          <Link href="/products" className="text-red-400 hover:text-red-300 transition-colors">
            Browse Products →
          </Link>
        </div>
      ) : (
                ...
                {orders.map((order) => (
                  <tr key={order.id} className="hover:bg-white/2 transition-colors">
                    <td className="px-4 py-4">
                      <Link href={`/account/orders/${order.id}`} className="text-[var(--text-primary)] font-mono font-bold text-sm hover:text-red-400 transition-colors">
                        {order.orderNumber}
                      </Link>
                    </td>
                    ...
                    <td className="px-4 py-4">
                      {order.invoice ? (
                        <Link href={`/account/invoices/${order.invoice.id}`} className="text-red-400 hover:text-red-300 text-xs font-semibold transition-colors">
                          Download
                        </Link>
                      ) : (
