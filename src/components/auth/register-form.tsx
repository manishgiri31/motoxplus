...
export function RegisterForm() {
  ...
  if (status === "success") {
    return (
      <div className="glass border border-[var(--border-color)] rounded-2xl p-8 text-center">
        ...
        <Link href={`/verify-email?userId=${userId}`} className="inline-block bg-red-600 hover:bg-red-700 text-white font-bold py-3 px-8 rounded-xl transition-colors uppercase tracking-wider text-sm">
          Verify Email Now
        </Link>
      </div>
    );
  ...
  const inputCls = "w-full themed-input border focus:border-red-600/60 rounded-xl px-4 py-3 text-sm outline-none transition-colors";
    ...
    <div className="glass border border-[var(--border-color)] rounded-2xl p-8">
      ...
      {step === 1 && (
        <form onSubmit={handleStep1} className="space-y-4">
          ...
          <button type="submit" className="w-full bg-red-600 hover:bg-red-700 text-white font-bold py-4 rounded-xl transition-colors uppercase tracking-wider text-sm">
      ...
      {step === 2 && (
        <form onSubmit={handleStep2} className="space-y-4">
          ...
          <div className="flex gap-3 mt-2">
            ...
            <button type="submit" disabled={status === "loading"} className="flex-[2] inline-flex items-center justify-center gap-2 bg-red-600 hover:bg-red-700 disabled:opacity-50 text-white font-bold py-4 rounded-xl transition-colors uppercase tracking-wider text-sm">
